#
#........Weighted Oblique Decision Trees (Some part of the code has been shared by the author)...............
#
import numpy as np
import random
...
class node(object):
	...
	def find_best_split(self, max_features='sqrt'):
		...
		def func(a):
			...
			# fun = w_L.sum() * compute_entropy(Y, w_L) + w_R.sum() * compute_entropy(Y, w_R)
			return fun
...
class BaseObliqueTree(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        ###########
        self.root_node = node(1, SplitQuestion(), np.arange(self.sampleNum, dtype=np.uint32), self.X, self.Y,
                              self.classNum)
        self.leaf_num = 1
